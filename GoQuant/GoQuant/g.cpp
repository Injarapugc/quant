...
std::string getOrderIdFromResponse(const std::string& response) {
    ...
    return "";  // Return empty string if order ID not found
}
std::string placeOrder(const std::string& price, const std::string& accessToken, const std::string& amount, const std::string& instrument) {
    json payload = {
        {"jsonrpc", "2.0"},
        {"method", "private/buy"},
        {"params", {
...
while (true) {
    ...
    switch(instrumentChoice) {
        case 1: instrument = "ETH-PERPETUAL"; break;
        case 2: instrument = "BTC-PERPETUAL"; break;
        case 3: instrument = "SOL-PERPETUAL"; break;
        case 4: instrument = "XRP-PERPETUAL"; break;
        case 5: instrument = "BNB-PERPETUAL"; break;
        default:
            std::cout << "Invalid choice. Defaulting to ETH-PERPETUAL\n";
    ...
    std::cout << "Enter amount: ";
    std::getline(std::cin, amount);
    std::cout << "Place Order Response: " << sendRequest("https://test.deribit.com/api/v2/private/sell", 
        json{
            {"jsonrpc", "2.0"},
            {"method", "private/sell"},
            {"params", {
